...
class TracerAlumniController extends Controller
{
    public function index(Request $request)
    {
        if (auth()->user()->hasRole('admin')) {
            $regency = Regency::all()->map(function ($data) use ($request) {
                ...
                $totalResiden = User::whereHas('ujian', function ($query) {
                    ...
                    $query->where('kabupaten_id', $data->id)->whereNotNull('tempat_bertugas');
                    if ($request->prodiId) {
        ...
        if (auth()->user()->hasRole('prodi')) {
            $regency = Regency::all()->map(function ($data) {
                $totalResiden = User::role('residen')->whereHas('biodata', function ($query) use ($data) {
    ...
    {
        $regency = ResidenBiodata::whereNotNull('tempat_bertugas')->with('user.ujian', 'kabupaten', 'prodi')->where('kabupaten_id', $id);
