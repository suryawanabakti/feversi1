...
export default function ResidenComparison() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await axios.post("/api/comparison/upload", formData, {
        headers: {
  ...
  const handleExport = async () => {
    try {
      const res = await axios.get("/api/comparison/export", {
        responseType: "blob",
      })
  ...
  const handleDownloadTemplate = async () => {
    try {
      const res = await axios.get("/api/comparison/template", {
        responseType: "blob",
      })
  ...
  const getCurrentData = () => {
    ...
    switch (activeTab) {
      case "missing":
        data = comparisonResult.mahasiswa_not_in_pegawai || []
        break
      case "extra":
        data = comparisonResult.pegawai_not_in_mahasiswa || []
        break
      case "matching":
              ...
              <Col md={3}>
                <Card className="text-center border-primary">
                  <Card.Body>
                    ...
                    <h4>{comparisonResult.summary.total_mahasiswa}</h4>
            ...
            <div>
              ...
              <div className="mb-3">
                ...
                >
                  ...
                  Tidak di Mahasiswa ({comparisonResult.summary.missing_from_mahasiswa})
                </Button>
                <Button
                  ...
                  <h5 className="mb-0">
                    {activeTab === "missing" && (
                      <>
                        ...
                        Mahasiswa yang TIDAK ADA di Data Kepegawaian
                      </>
                    )}
                    {activeTab === "extra" && (
                      <>
                        <Users className="me-2 text-warning" size={20} />
                        Pegawai yang TIDAK ADA di Data Mahasiswa
                      </>
                    )}
                    {activeTab === "matching" && (
                      <>
                    ...
                    <Table striped hover className="mb-0">
                      <thead className="table-dark">
                        <tr>
                          ...
                          {activeTab === "missing" && (
                            <>
                              {renderTableHeader("username", "NIM")}
                              {renderTableHeader("nama", "Nama")}
                            </>
                          )}
                          {activeTab === "extra" && (
                            <>
                              {renderTableHeader("nim", "NIM")}
                              {renderTableHeader("nama", "Nama")}
                            </>
                          )}
                          {activeTab === "matching" && (
                            <>
                              {renderTableHeader("nim", "NIM")}
                              {renderTableHeader("nama", "Nama Mahasiswa")}
                            </>
                          )}
                        </tr>
                      </thead>
                          ...
                          getCurrentPageData().map((item, index) => (
                            <tr key={index}>
                              ...
                              {activeTab === "missing" && (
                                <>
                                  <td>
                                    <strong className="text-danger">{item.username}</strong>
                                  </td>
                                  <td>{item.name}</td>
                                </>
                              )}
                              {activeTab === "extra" && (
                                <>
                                  <td>
                                    <strong className="text-warning">{item.nim}</strong>
                                  </td>
                                  <td>{item.nama}</td>
                                </>
                              )}
                              {activeTab === "matching" && (
                                <>
                                  <td>
                                    <strong className="text-success">{item.nim}</strong>
                                  </td>
            ...
            <Card className="text-center py-5">
              <Card.Body>
                ...
                <p className="text-muted">
                  Upload file Excel untuk memulai perbandingan data kepegawaian dengan mahasiswa
                </p>
                <Button variant="primary" onClick={handleShowUpload}>
                  <Upload className="me-2" size={16} />
