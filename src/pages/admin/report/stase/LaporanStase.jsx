...
export default function LaporanStase() {
  const { user } = useAuthContext();
  const [dataRs, setDataRs] = useState([]);
  const [rumahSakit, setRumahSakit] = useState(
    user.username == "rsunhas" ? 4 : ""
  );
  ...
  const getRumahSakit = async () => {
    try {
      const res = await axios.get("/api/ref/get-rumahsakit");
  ...
  const handlePrint = async (e) => {
    ...
    try {
      const res = await axios.get(
        `/api/laporan-stase/${rumahSakit ? rumahSakit : "semua"}/${bulan ? bulan : "semua"
        }/${tahun ? tahun : "semua"}`
      );
  ...
  const detailResidenByProdi = async (prodiId, prodiName) => {
  ...
  const handleDownloadReport = () => {
    const url = `${baseurl}/api/laporan-stase/${rumahSakit}/${bulan}/${tahun}/${format}/cetak`;
                  ...
                  <Col md={6}>
                    <div className="form-group mb-3">
                      <label className="form-label fw-bold">
                        <i className="fas fa-hospital me-1 text-success"></i>
                      ...
                      <select
                        ...
                        value={rumahSakit}
                        disabled={user.username == "rsunhas"}
                      >
                          ...
                          <tr key={d.id}>
                            ...
                            <td className="text-center">
                              <button
                                ...
                                onClick={() =>
                                  detailResidenByProdi(d.id, d.name)
                                }
