...
const ShowLaporanProvinsi = () => {
  ...
  const fetchDatas = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/laporan-provinsi/${id}`, {
  ...
  const provinceName = datas.length > 0 ? datas[0].namaProvinsi : "Wilayah";
    ...
    <div className="main-content">
      <section className="section">
        <div className="section-header border-bottom-0 pb-0 bg-transparent shadow-none d-flex justify-content-between align-items-center">
          <div>
            <h1 className="text-dark" style={{ fontWeight: 800, fontSize: '1.75rem' }}>Detail Sebaran</h1>
            <p className="text-muted mt-1">
              Wilayah <strong>{provinceName}</strong> &bull; {statusLabel}
            </p>
          </div>
              ...
              <Card className="border-0 shadow-sm overflow-hidden" style={{ borderRadius: '15px' }}>
                <Card.Header className="bg-white border-bottom-0 py-3 px-4 d-flex justify-content-between align-items-center flex-wrap">
                  <h4 className="m-0 text-dark" style={{ fontSize: '1.1rem', fontWeight: 700 }}>
                    Populasi di {provinceName} ({totalItems})
                  </h4>
                    ...
                    <div className="table-responsive">
                      <Table hover className="mb-0">
                        <thead className="bg-light">
                          <tr>
                            <th className="px-4 py-3 border-0 text-muted small text-uppercase fw-bold">NIM / Username</th>
                            <th className="px-4 py-3 border-0 text-muted small text-uppercase fw-bold">Nama Lengkap</th>
                            <th className="px-4 py-3 border-0 text-muted small text-uppercase fw-bold">Program Studi</th>
                            <th className="px-4 py-3 border-0 text-muted small text-uppercase fw-bold">Lokasi Tugas</th>
                          </tr>
                        </thead>
                        <tbody>
...
export default ShowLaporanProvinsi;
