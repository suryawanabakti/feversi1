...
/**
 * Serkom Page Component
 * 
 * Manages display and deletion of Sertifikat Kompetensi (Serkom).
...
const Serkom = () => {
  ...
  const columns = [
    { field: "name", headerName: "Nama Sertifikat", width: "40%" },
    {
      field: "created_at",
  ...
  const getSerkom = async () => {
    setFetching(true);
    try {
      const res = await axios.get("/api/serkom");
      // Handle Laravel Resource collection structure
      setSerkom(res.data.data || res.data);
    } catch (err) {
      console.error(err);
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm(`Hapus ${pilihan.length} sertifikat yang dipilih?`)) return;

    setLoading(true);
    try {
      await axios.post("/api/serkom/delete-all", { pilihan });
      toast.success("Berhasil hapus sertifikat yang dipilih");
      getSerkom();
      setPilihan([]);
    } catch (err) {
      console.error(err);
      toast.error(err.response?.data?.message || "Gagal hapus sertifikat");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="section-body">
          <Card className="border-0 shadow-sm">
            <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
              <h5 className="mb-0 text-primary">Daftar Sertifikat</h5>
              <div>
                <Button
                  variant="primary"
                  onClick={handleShow}
