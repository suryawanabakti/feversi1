 ...
 * KHS Page Component
 * 
 * Manages display and deletion of Kartu Hasil Studi (KHS).
...
const Khs = () => {
  ...
  const getKhs = async () => {
    setFetching(true);
    try {
      const res = await axios.get("/api/khs");
      // Handle Laravel Resource collection structure
      setKhs(res.data.data || res.data);
    } catch (err) {
      console.error(err);
      toast.error("Gagal Mengambil Data KHS");
    } finally {
      setFetching(false);
    }
  };
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Gagal hapus KHS");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="section-body">
          <Card className="border-0 shadow-sm">
            <Card.Header className="bg-white py-3 d-flex justify-content-between align-items-center">
              <h5 className="mb-0 text-primary">Daftar KHS</h5>
              <div>
                <Button
                  variant="primary"
                  onClick={handleShow}
