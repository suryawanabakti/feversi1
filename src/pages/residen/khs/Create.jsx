...
/**
 * Create KHS Component
 * 
 * Modal form for uploading new KHS (Kartu Hasil Studi).
 */
const Create = ({ show, handleClose, getKhs }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("/api/khs", formData, {
      ...
      if (err.response?.status === 422) {
        ...
        toast.error(err.response?.data?.message || "Gagal mengunggah KHS");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal show={show} onHide={handleClose} centered backdrop="static" size="lg">
      <Modal.Header closeButton className="bg-primary text-white">
        <Modal.Title className="h5">Tambah KHS</Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
        <Modal.Body className="py-4">
          ...
          <Form.Group className="mb-0 text-left">
            <Form.Label className="font-weight-bold">
              File KHS <span className="text-danger">*</span>
            </Form.Label>
            <Form.Control
              type="file"
              accept=".jpg,.jpeg,.png,.pdf"
              onChange={(e) => setKhs(e.target.files[0])}
              isInvalid={!!errors.khs}
            />
