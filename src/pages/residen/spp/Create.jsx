...
/**
 * Create SPP Component
 * 
 * Modal form for uploading new SPP (Surat Pembayaran Pendidikan).
 */
const Create = ({ show, handleClose, getSpp }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("/api/spp", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      toast.success("SPP berhasil diunggah");
      resetForm();
      handleClose();
      getSpp();
    } catch (err) {
      ...
      if (err.response?.status === 422) {
        ...
        toast.error(err.response?.data?.message || "Gagal mengunggah SPP");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal show={show} onHide={handleClose} centered backdrop="static" size="lg">
      <Modal.Header closeButton className="bg-primary text-white">
        <Modal.Title className="h5">Tambah SPP</Modal.Title>
      </Modal.Header>
      <Form onSubmit={handleSubmit}>
        <Modal.Body className="py-4">
          ...
          <Form.Group className="mb-0 text-left">
            <Form.Label className="font-weight-bold">
              File SPP <span className="text-danger">*</span>
            </Form.Label>
            <Form.Control
              type="file"
              accept=".jpg,.jpeg,.png,.pdf"
              onChange={(e) => setSpp(e.target.files[0])}
              isInvalid={!!errors.spp}
            />
